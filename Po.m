...
%         imageData=double(imread(image));
        s=0.2;
        lambda=0.0001;
        %extraction des diff�rents canaux
        red=imageData(:,:,1);
        green=imageData(:,:,2);
        blue=imageData(:,:,3);
        ...
        [M,N,~]=size(imageData);
        %%calcule de l'inverse, prendre la partie r�elle
...
%         subplot(2,2,2), imshow(result/255), title('R�sultat');
%         subplot(2,2,3), imhist(mean(imageData(:,:,1),3)/255), title('Histogramme avant');
%         subplot(2,2,4), imhist(mean(result(:,:,1),3)/255), title('Histogramme apr�s');
